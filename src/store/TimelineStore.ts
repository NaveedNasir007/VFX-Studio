...
export const useTimelineStore = create<TimelineState>((set, get) => ({
  ...
  addVoiceover: async (mediaUri: string, startMs: number, durationMs: number) => {
    ...
    if (voTrackIndex === -1) {
      tracks.push({ id: `voiceover_${uuid.v4()}`, type: 'voiceover', clips: [] });
      voTrackIndex = tracks.length - 1;
    }
    ...
    const voTrack = { ...tracks[voTrackIndex] };
    voTrack.clips = [...voTrack.clips, voClip];
    tracks[voTrackIndex] = voTrack;
  ...
  addCaptions: async (captionsList: { text: string; start: number; duration: number }[]) => {
    ...
    const captionClips: Clip[] = captionsList.map(cap => ({
      id: uuid.v4() as string,
      type: 'caption',
      start: cap.start,
      duration: cap.duration,
      mediaStart: 0,
      mediaDuration: cap.duration,
      textData: {
        text: cap.text,
        color: '#FFFFFF',
        fontSize: 24,
    ...
    captionTrack.clips = captionClips;
